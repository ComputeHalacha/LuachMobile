...
export default class DataUtils {
    static databasePath = !!GLOBALS ? GLOBALS.DB_TEMPLATE_PATH : null;
    ...
    static async setCurrentDatabasePath(newPath: string) {
        let appData: AppData;
        // Save the path to the new database path to local storage
    ...
    static async LocationFromDatabase(locationId: number) {
        ...
        try {
            const ls = await DataUtils.queryLocations("locationId=?", [
                locationId,
            ]);
            if (ls.length > 0) {
                [location] = ls;
         ...
         Starting execution of ${sql} - with values ${values}`
                ...
                if (results && results.length > 0) {
                    ...
                    if (sql.toUpperCase().startsWith("SELECT")) {
                        log(
                            `0121 - sql executed successfully - ${resultSet.rows.length} rows returned`
                        );
                        for (let i = 0; i < resultSet.rows.length; i++) {
                            resultsList.push(resultSet.rows.item(i));
                        }
                    } else if (resultSet.rowsAffected) {
                        log(
                            `0122 - sql executed successfully - ${resultSet.rowsAffected} rows affected`
                        );
                    } else {
                        log(
                    ...
                    warn(
                        `0124 - sql executed without generating an error but no response or results returned at all`
                    );
                }
            } catch (err) {
                warn(
                    `0125 - error opening database - ${DataUtils.databasePath}`
                );
                error(err);
                if (database !== undefined)
                    await DataUtils.closeDatabase(database);
            }
        }
    ...
    static async assureDatabaseExists() {
        if (
            DataUtils.databasePath &&
            (await fileExists(DataUtils.databasePath as string))
        ) {
            return true;
        }
        try {
            if (!(await fileExists(GLOBALS.DB_TEMPLATE_PATH))) {
                throw `${DataUtils.databasePath} was not found, and database template is missing ${GLOBALS.DB_TEMPLATE_PATH}`;
            }

            const newPath = getNewDatabaseFullFileName();
            log(
                `${DataUtils.databasePath} was not found.
        Starting copy from ${GLOBALS.DB_TEMPLATE_PATH} to ${newPath}`
            );
            try {
                if (!(await fileExists(GLOBALS.DB_TEMPLATE_PATH))) {
                    error(`${GLOBALS.DB_TEMPLATE_PATH} was not found`);
                    return false;
                }
                await copyFile(GLOBALS.DB_TEMPLATE_PATH, newPath);
                await this.setCurrentDatabasePath(newPath);
                return true;
            } catch (err) {
                error(err);
                error(
                    `Failed to copy from ${GLOBALS.DB_TEMPLATE_PATH} to ${newPath} due to error ${err.message}`
                );
                return false;
            }
        } catch (err) {
...
export function getAppBundleIdAndroid() {
    if (GLOBALS.IS_ANDROID) {
        return fs.DocumentDirectoryPath.replace(/.+\/(.+?)\/files/, "$1");
    }
}
