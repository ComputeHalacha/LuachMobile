        ...
        switch (this.state.dataSet) {
            ...
            case 'Events':
                ...
                for (let occ of this.appData.UserOccasions) {
                    csv += `"${occ.jdate.toShortString(
                        false
                    )}","${occ.sdate.toLocaleDateString()}","${occ.toString(
                        true
                    )}","${occ.comments}","${occ.color}"\r\n`;
                }
                break;
            case 'Kavuahs':
                csv =
                ...
                for (let kavuah of this.appData.KavuahList) {
                    csv += `"${kavuah.toString()}","${kavuah.settingEntry.toLongString()}","${yon(
            ...
            case 'Settings': {
                csv =
                    ...
                    '"Show Ignored Kavuahs","No Flags Right After Entry","Hide Help","Discreet"\r\n' +
                    ...
                    )}","${settings.numberMonthsAheadToWarn.toString()}"` +
                    `,"${yon(settings.calcKavuahsOnNewEntry)}","${yon(
    ...
    getHtmlText() {
        ...
        switch (this.state.dataSet) {
            ...
            case 'Kavuahs':
                for (let kavuah of this.appData.KavuahList) {
                    counter++;
                    html += `<p>${counter.toString()}. ${kavuah
                        .toLongString()
                        .replace(/\n/g, '<br />')}</p><hr />`;
                }
                break;
            case 'Settings':
                html +=
                    ...
                    `<p><b>Keep Onah Beinonis (30, 31 and Yom HaChodesh) for a full 24 Hours</b><br />${yon(
                    ...
                    `<p><b>Continue incrementing Dilug Yom Hachodesh Kavuahs into another month</b><br />${yon(
                        settings.dilugChodeshPastEnds
                    )}<hr /></p>` +
                    `<p><b>Calculate Haflagas by counting Onahs</b><br />${yon(
                        settings.haflagaOfOnahs
                    )}<hr /></p>` +
                    `<p><b>Flag Kavuahs even if not all the same Onah</b><br />${yon(
                        settings.kavuahDiffOnahs
                    )}<hr /></p>` +
                    `<p><b>Number of Months ahead to warn</b><br />${settings.numberMonthsAheadToWarn.toString()}<hr /></p>` +
                    `<p><b>Automatically Calculate Kavuahs upon addition of an Entry</b><br />${yon(
                        settings.calcKavuahsOnNewEntry
                    )}<hr /></p>` +
                    `<p><b>Show Entry, Hefsek Tahara and Mikva information</b><br />${yon(
                        settings.showEntryFlagOnHome
                    )}<hr /></p>` +
                    `<p><b>Show flags for problem dates on Main Screen</b><br />${yon(
                    ...
                    `<p><b>Don't show Flagged dates for a week after Entry</b><br />${yon(
                        settings.noProbsAfterEntry
                    )}<hr /></p>` +
            ...
            case 'Flagged Dates':
                for (let probOnah of this.appData.ProblemOnahs) {
                    counter++;
                    html += `<p>${counter.toString()}. ${probOnah
                        .toString()
                        .replace(/\n/g, '<br />&nbsp;&nbsp;')}</p><hr />`;
                }
                break;
            case 'Zmanim - ' + this.jdate.toShortString(): {
                ...
                html +=
                    ...
                    details
                        .map(
                            d => `<p><b>${d.title}</b><br />${d.value}</hr></p>`
                        )
                        .join('') +
                    '<hr />';
                break;
            ...
            case 'Zmanim - ' + this.jdate.monthName(): {
                html +=
                    '<p>Please find attached a spreadsheet file with the Zmanim \
                    for the month of <b>' +
                    this.jdate.monthName() +
                    '</b> for <b>' +
                    settings.location.Name +
                    '</b></p>';
                break;
    ...
    async doEmail() {
        ...
        Mailer.mail(
            ...
            error => {
                if (error) {
                    popUpMessage(
                        'We are very sorry, but the email could not be sent.'
                    );
                }
            }
        );
                ...
                <View style={{ flexDirection: 'row', flex: 1 }}>
                    ...
                    <ScrollView style={{ flex: 1 }}>
                        ...
                        <View
                            ...
                            <Text>
                                When you press the "Export to Email" button
                                below, your default email client will open up in
                                "compose" mode, with an email containing{' '}
                                {dataDesc}.{'\n\n'}A spreadsheet with {dataDesc}{' '}
                                will also be attached to the email.
                                {!this.state.dataSet.startsWith('Zmanim') && (
                                    <Text>
                                        {'\n\n'}
                                        It is advisable to send the email to
                                        yourself and to keep it as a backup of
                                        your data.
                                        {'\n\n'}
                                        NOTE: It is not (yet) possible to IMPORT
                                        data into Luach.
                                        {'\n'}
                                        If you need to restore your data, it
