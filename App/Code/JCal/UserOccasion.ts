...
class UserOccasion {
    ...
    toString(noOriginalDate: boolean) {
        switch (this.occasionType) {
            ...
            case UserOccasionTypes.HebrewDateRecurringYearly:
                ...
                }Yearly event on the ${Utils.toSuffixed(
                    this.jdate.Day
     ...
     */
    static getOccasionsForDate(
        jdate: JDate,
        allOccasions: Array<UserOccasion>
    ) {
        return (
            !!allOccasions &&
            allOccasions.filter((o) => {
                const oJDate = o.jdate;
                switch (o.occasionType) {
                    case UserOccasionTypes.OneTime:
                        return o.dateAbs === jdate.Abs;
                    case UserOccasionTypes.HebrewDateRecurringYearly:
                        const occMonth = oJDate.Month,
                            currMonth = jdate.Month;
                        if (occMonth >= 12 && currMonth >= 12) {
                            const isOccLeap = JDate.isJdLeapY(oJDate.Year),
                                isCurrLeap = JDate.isJdLeapY(jdate.Year);
                        ...
                        return (
                            occMonth === currMonth && oJDate.Day === jdate.Day
                        );
                    case UserOccasionTypes.HebrewDateRecurringMonthly:
