...
export default class RemoteBackupScreen extends React.Component {
    ...
    async componentDidMount() {
        ...
        if (
            this.state.localStorage.remoteUserName &&
            this.state.localStorage.remotePassword
        ) {
    ...
    async restoreFromBackup() {
        if (
            ...
            const {
                ...
                appData,
                message,
            } = await this.remoteBackup.restoreBackup();
            if (success && appData) {
                this.update(appData);
                Alert.alert(
    ...
    render() {
        const remoteUserName =
                this.state.enteredRemoteUserName ||
                this.state.localStorage.remoteUserName,
            remotePassword =
                this.state.enteredRemotePassword ||
                this.state.localStorage.remotePassword;
                ...
                <View style={{ flexDirection: 'row', flex: 1 }}>
                    ...
                    <ScrollView style={{ flex: 1 }}>
                        ...
                        <View
                            ...
                            <Text>
                                When you press the "Backup My Data" button
                                below, your data will be backed up to the Luach
                                servers.
                                {'\n\n'}
                                This data includes all your Entries, Kavuahs,
                                Settings, Events etc.
                                {'\n\n'}
                                Important note, this action will overwrite any
                                previous backups that you have done.
                                {'\n\n'}
                                If you need multiple backups, you can create
                                another account be entering different
                                credentials above and pressing "Create Account".
                                {'\n\n'}
                                Each account can have one data backup.
                                {'\n\n'}
                                You can retrieve and restore your backup data by
                                entering the correct credentials above, and
                                pressing on "Restore from Backup" below.
                                {'\n\n'}
                                WARNING: Restoring your data, will overwrite any
                                existing data currently stored locally in Luach.
                            </Text>
                        </View>
                        <View
                            style={[
